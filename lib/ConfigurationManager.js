...
module.exports = class ConfigurationManager {
    ...
    _getConnectionConfigurations () {
        ...
        return require(this.ccfp).sort((a,b)=>b.sort-a.sort)
    }
    ...
    addConnectionConfiguration (name, description, user, server, port, path, authentication, password, remoteCommand, sort) {
    ...
    // 扩展
    editConnectionConfiguration (uuid, name, description, user, server, port, path, authentication, password, remoteCommand, sort) {
        let cc = new ConnectionConfiguration({
            'uuid': uuid,
            'name': name,
            'description': description,
