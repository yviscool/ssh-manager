...
const numberPrompt = require('inquirer-number-plus')
const prompt = inquirer.createPromptModule()
prompt.registerPrompt('number',numberPrompt)
...
module.exports = class ConfigurationCommand extends SubCommand {
    ...
    static async _getConnectionConfigurationProperty(message, name, defaultValue = null, validationFunction = null, type = 'input') {
        ...
        if (validationFunction !== null) {
            ...
            if (errors.length !== 0) {
                ...
                property[name] = await ConfigurationCommand._getConnectionConfigurationProperty(message, name, defaultValue, validationFunction, type)
            }
        }
    ...
    static async _createAction() {
        ...
        let sort = await ConfigurationCommand._getConnectionConfigurationProperty('Sort', 'sort', '1', CCValidator.validateSort, 'number')
        ...
        let password = "";
        // 扩展
        if (authentication.type === ConnectionConfiguration.AUTHENTICATION_TYPE_PASSWORD()) {
            password = await ConfigurationCommand._getConnectionConfigurationProperty('Password', 'password', 'root', CCValidator.validatePassword, 'password')
            await ConfigurationCommand._getConnectionConfigurationProperty('Confirm password', 'password', 'root',
                password2 => CCValidator.validateConfirmPassword(password, password2),
                'password'
            )

        }
        let remoteCommand = await ConfigurationCommand._getConnectionConfigurationProperty('RemoteCommand', 'remoteCommand', `echo 呵呵`, CCValidator.validateRemoteCommand)

        cm.addConnectionConfiguration(name, description, user, server, port, path, authentication, password, remoteCommand, sort)
    }
    ...
    static async _editAction() {
        ...
        // 扩展
        if (authentication.type === ConnectionConfiguration.AUTHENTICATION_TYPE_PASSWORD()) {
            password = await ConfigurationCommand._getConnectionConfigurationProperty('Password', 'password',
                '********',
                CCValidator.validatePassword,
                'password',
            )
            await ConfigurationCommand._getConnectionConfigurationProperty('Confirm password', 'password',
                '********',
                password2 => CCValidator.validateConfirmPassword(password, password2),
                'password',
            )
        ...
        let remoteCommand = await ConfigurationCommand._getConnectionConfigurationProperty('RemoteCommand', 'remoteCommand', cc.remoteCommand, CCValidator.validateRemoteCommand)

        cm.editConnectionConfiguration(cc.uuid, name, description, user, server, port, path, authentication, password, remoteCommand, sort)
    }
